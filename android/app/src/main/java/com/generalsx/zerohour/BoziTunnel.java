...
public final class BoziTunnel {
    ...
    /**
     * Останавливает туннель и освобождает место в комнате.
     *
     * <p>Долгая операция: сетевая часть дожидается своих фоновых циклов
     * (опрос состояния держится до 25 секунд) и только потом сообщает
     * серверу о выходе. Поэтому саму остановку делаем БЕЗ замка: держать
     * его всё это время значило бы заблокировать {@link #status()}, который
     * экран зовёт из главного потока, — и приложение «не отвечало» на кнопке
     * «Выйти». Ссылку на сессию забираем под замком, а работу делаем снаружи.
     * Звать из фонового потока.
     */
    public void stop() {
        glnet.Session finished;
        synchronized (this) {
            finished = session;
            session = null;
            tunnelStarted = false;
